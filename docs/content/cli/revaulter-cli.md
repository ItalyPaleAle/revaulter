...
Mount a volume at `/data` to keep the trust store between runs. Pin the server's anchor once, with `-it` so the CLI can ask for confirmation:
...
Every later command reuses that pin:

```bash
docker run --rm \
  -v revaulter-trust:/data \
  ghcr.io/italypaleale/revaulter-cli:2 encrypt \
  --server https://revaulter.example.com \
  --request-key AbCdEf0123456789GhIj \
  --key-label my-key \
  --algorithm A256GCM \
...
Commands that read a file or write their output to one also need the directory mounted, for example `-v "$PWD:/work"`. The container runs as a non-root user (UID 65532), so those files must be readable, and writable, by that user.

    ...
    switch (req.operation) {
        case 'sign': {
            ...
            switch (req.algorithm) {
                case 'ES256':
                    // For ES256, the value is pre-hashed always
                    if (value.length !== 32) {
                ...
                case 'Ed25519':
                    // For Ed25519 the value is not pre-hashed
                    signature = await signMessageEd25519(secretKey, value)
                    break

                case 'Ed25519ph':
                    // Variant of Ed25519 where the value is prehashed
                    // Note that this is a different algorithm than Ed25519, and signatures are incompatible
                    if (value.length !== 64) {

...
export async function deriveSigningKeyPair(params: {
    ...
    // Handle Ed25519 (incl. ph variant)
    const seedBits = await crypto.subtle.deriveBits(
        { name: 'HKDF', hash: 'SHA-256', salt: asBuf(new Uint8Array()), info: asBuf(info) },
        ikm,
        256
...
const ed25519phScheme = (() => {
    ...
    return eddsa(ed25519.Point, awasmSha512, {
        adjustScalarBytes,
...
export async function signingJwkToPem(jwk: V2SigningJwk): Promise<string> {
    let spki: Uint8Array
    if (jwk.kty === 'EC') {
        // Import into a CryptoKey and export for simplicy and correctness
        ...
        // CryptoKey doesn't support Ed25519 so we just write the SPKI header and encoded bytes directly

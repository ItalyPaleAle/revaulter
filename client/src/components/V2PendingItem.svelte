...
async function buildResponseEnvelope(req: V2RequestDetail) {
    ...
    const value = b64urlToBytes(input.value)
    const aad = b64urlToBytes(input.additionalData)
        ...
        case 'wrapkey': {
            ...
            resultPlain = new TextEncoder().encode(
                JSON.stringify({
                    ...
                    operation: req.operation,
                    algorithm: req.algorithm,
                    value: bytesToB64url(split.data),
                    nonce: bytesToB64url(nonce),
                    tag: bytesToB64url(split.tag),

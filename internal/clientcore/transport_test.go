...
func TestDecryptV2ResponseEnvelope(t *testing.T) {
	...
	key, err := DeriveTransportKey(combined, "state-1")
	require.NoError(t, err)
	...
	require.NoError(t, err)
	aad := BuildTransportAAD("state-1", "encrypt", "A256GCM")
	plain := []byte(`{"ok":true}`)
	...
	out, err := DecryptResponseEnvelope("state-1", kp, &protocolv2.ResponseEnvelope{
...
func TestDeriveV2TransportKeyDeterministic(t *testing.T) {
	...
	k1, err := DeriveTransportKey(sum[:], "x")
	require.NoError(t, err)
	k2, err := DeriveTransportKey(sum[:], "x")
	require.NoError(t, err)
...
func TestBuildTransportAADMatchesBrowserOrdering(t *testing.T) {
	require.Equal(
		t,
		[]byte("algorithm=A256GCM\noperation=encrypt\nstate=state-1\nv=1"),
		BuildTransportAAD("state-1", "encrypt", "A256GCM"),
	)
}

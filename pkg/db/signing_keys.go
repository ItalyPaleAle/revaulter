...
// PublishedSigningKey represents a public signing key published by a user
...
// Re-publishing different material under the same label replaces the previous row
func (s *SigningKeyStore) Upsert(ctx context.Context, in UpsertPublishedSigningKeyInput) error {
	...
	_, err := s.db.Exec(ctx,
		`INSERT INTO v2_published_signing_keys
			(id, user_id, algorithm, key_label, jwk, pem, created_at, updated_at)
...
func (s *SigningKeyStore) GetByID(ctx context.Context, id string) (*PublishedSigningKey, error) {
	...
	err := s.db.
		QueryRow(ctx,
			`SELECT id, user_id, algorithm, key_label, jwk, pem, created_at, updated_at
				FROM v2_published_signing_keys WHERE id = $1`,
			id,
		).
		Scan(&rec.ID, &rec.UserID, &rec.Algorithm, &rec.KeyLabel, &rec.JWK, &rec.PEM, &createdAt, &updatedAt)
...
// ListForUser returns the user's published keys (metadata only, no JWK/PEM) so the settings UI can render the list without transferring key material
func (s *SigningKeyStore) ListForUser(ctx context.Context, userID string) ([]PublishedSigningKeyListItem, error) {
	rows, err := s.db.Query(ctx,
		`SELECT id, algorithm, key_label, created_at, updated_at
	...
	for rows.Next() {
		...
		var createdAt, updatedAt int64
		err = rows.Scan(&item.ID, &item.Algorithm, &item.KeyLabel, &createdAt, &updatedAt)
		if err != nil {
			return nil, err
		}

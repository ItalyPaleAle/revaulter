...
// It contains only data — no schema DDL — along with the list of applied migrations so that the target database can be migrated to the exact same version before the restore
package backup
...
type BackupFile struct {
	...
	// Migrations is the ordered list of migration names that had been applied to the source database at backup time
	// On restore this is used to bring the target database to the same schema version before inserting data
...
type TableBackup struct {
	...
	// Each element of Rows corresponds to this column list
	Columns []string `cbor:"c"`
	// Rows contains the data rows
	// Each inner slice is parallel to Columns
...
//
// The backup contains only data (no DDL)
// The migration version of the source database is embedded so the restore process can verify schema compatibility
...
// readMigrations queries the metadata table and returns the list of applied migration names stored under the "migrations" key
func readMigrations(ctx context.Context, conn *db.DB) ([]string, error) {
	...
	err = json.Unmarshal([]byte(raw), &migrations)
	if err != nil {
...
// dumpTable runs a SELECT over all tracked columns of spec.name and returns a TableBackup with normalised, CBOR-safe row data

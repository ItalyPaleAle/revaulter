...
func TestWebhook(t *testing.T) {
	...
	t.Run("format plain", basicTestFn(map[string]any{
		...
		requireBodyEqual(t, r.Body, "Received a request to encrypt using key label **mykey** for user **Alice**.\n\nOpen Revaulter: http://198.51.100.10/app\n\n(Client IP: 127.0.0.1)")
	}))
	...
	t.Run("format slack", basicTestFn(map[string]any{
		...
		require.Equal(t, "http://198.51.100.10/endpoint", r.URL.String())
		requireBodyEqual(t, r.Body, `{"text":"Received a request to encrypt using key label **mykey** for user **Alice**.\n[Open Revaulter](http://198.51.100.10/app)\n`+"`(Client IP: 127.0.0.1)`"+`"}`+"\n")
	}))
